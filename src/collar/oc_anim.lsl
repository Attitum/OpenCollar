...
AnimMenu(key kID, integer iAuth)
{
    ...
    if (g_bAnimLock) sPrompt += " is forbidden to change or stop poses on their own";
    else sPrompt += " is allowed to change or stop poses on their own";
    ...
    if (llGetInventoryType("~stiff") == INVENTORY_ANIMATION)
    {
        ...
        if (g_bPosture) sPrompt += " and has their neck forced stiff.";
        else sPrompt += " and can relax their neck.";
...
{ // Wrapper to send menu back to the AO's menu
...
{ // send string as "ON" / "OFF" saves 2 llToUpper
...
{ // Adds anim to queue, calls PlayAnim to play it, and calls AO as necessary
    if (llGetPermissions() & PERMISSION_TRIGGER_ANIMATION && llGetPermissions() & PERMISSION_OVERRIDE_ANIMATIONS)
    {
...
{ // Deals with removing anim from queue, calls UnPlayAnim to stop it, calls AO as nexessary
    if (llGetPermissions() & PERMISSION_TRIGGER_ANIMATION && llGetPermissions() & PERMISSION_OVERRIDE_ANIMATIONS)
    {
        if (llGetInventoryType(sAnim) == INVENTORY_ANIMATION)
        {
...
CreateAnimList()
{
    ...
    do
    {
        ...
        {
            if (!~llListFindList(["-1", "-2", "+1", "+2"], [llGetSubString(sName, -2, -1)])) g_lPoseList += sName;
        }
...
UserCommand(integer iNum, string sStr, key kID)
{
    ...
    else if (sCommand == "antislide")
    {
        if (iNum == CMD_OWNER || kID == g_kWearer)
        {
            ...
            else if (sValue == "off")
            {
                ...
                if (llList2String(lParams,2) == "") llMessageLinked(LINK_SET, NOTIFY, "1" + "AntiSlide is now disabled.", kID);
            }
            else if (sValue == "none")
            {
    ...
    else if (llGetInventoryType(sStr) == INVENTORY_ANIMATION)
    {
        ...
        else llMessageLinked(LINK_SET, NOTIFY, "0" + "%NOACCESS% to change pose", kID);
    }
}
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        else if (iNum == MENUNAME_RESPONSE)
        {
            if (llSubStringIndex(sStr, "Animations|") == 0)
            {
                ...
                if (llListFindList(g_lAnimButtons, [sChild]) == -1) g_lAnimButtons += sChild;
            }
        }
        ...
        else if (iNum == LM_SETTING_RESPONSE)
        {
            ...
            if (llGetSubString(sToken, 0, i) == g_sSettingToken)
            {
                ...
                if (sToken == "currentpose")
                {
                    ...
                    if(llGetListLength(g_lAnims)>0&&llList2String(g_lAnims,0)==g_sCurrentPose)
                    {}
                    else StartAnim(g_sCurrentPose);
                }
                ...
                {
                    if (llGetAnimationOverride("Standing") != "") g_bTweakPoseAO = (integer)sValue;
                }
            }
        ...
        else if (iNum == DIALOG_RESPONSE)
        {
            ...
            if (~iMenuIndex)
            {
                ...
                if (sMenuType == "Anim")
                {
                    ...
                    else if (~llListFindList(g_lAnimButtons, [sMessage])) llMessageLinked(LINK_SET, iAuth, "menu " + sMessage, kAv);
        ...
        else if(iNum == 0)
        {
            if(sMsg == "do_move" && !g_iIsMoving)
            {
                ...
                for(i=0;i<end;i++)
                {
                    ...
                    if(llGetInventoryType(item)==INVENTORY_SCRIPT && item!=llGetScriptName()) llRemoveInventory(item);
                    else if(llGetInventoryType(item)!=INVENTORY_SCRIPT)
                    {
                        ...
                        else llOwnerSay("Item '"+item+"' is no-copy and can not be moved! Please move it manually!");
                    }
                }
