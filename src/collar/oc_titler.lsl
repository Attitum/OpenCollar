...
ScanFloatText()
{
    ...
    for(i=0;i<=end;i++)
    {
        ...
        if(llSubStringIndex(llList2String(Params,0), "FloatText")!=-1) g_iTextPrim = i;
        if(llSubStringIndex(llList2String(Params,1), "FloatText")!=-1) g_iTextPrim=i;
    }
    ...
    link_message(integer iSender,integer iNum,string sStr,key kID)
    {
        ...
        else if(iNum == DIALOG_RESPONSE)
        {
            ...
            if(iMenuIndex!=-1)
            {
                ...
                {
                    if(sMsg == "White") g_vColor = <1,1,1>;
                    else if(sMsg == "Green") g_vColor = <0,1,0>;
                    else if(sMsg == UPMENU)
                    {
                        Menu(kAv, iAuth);
                        iRespring=FALSE;
                    }
                    else if(sMsg == "Yellow") g_vColor = <1,1,0>;
                    else if(sMsg == "Cyan") g_vColor = <0,1,1>;
                    else if(sMsg == "Blue") g_vColor = <0,0,1>;
                    else if(sMsg == "Pink") g_vColor = <1,0,1>;
                    else if(sMsg == "Orange")g_vColor = <1,0.5,0>;
                    else if(sMsg == "Custom")
                    {
        ...
        else if(iNum == LM_SETTING_RESPONSE)
        {
            ...
            {
                ScanFloatText();
                if(llList2String(lSettings,1) == "show") g_iShow=TRUE;
                else if(llList2String(lSettings,1) == "offset") g_iOffset=(integer)llList2String(lSettings,2);
                else if(llList2String(lSettings,1) == "title") g_sTitle = llBase64ToString(llList2String(lSettings,2));
                else if(llList2String(lSettings,1)=="color") g_vColor=(vector)llList2String(lSettings,2);
                Titler();
            }
        }
